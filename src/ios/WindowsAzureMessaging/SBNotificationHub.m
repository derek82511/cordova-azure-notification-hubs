...
- (void) registerNativeWithDeviceToken:(NSData*)deviceTokenData tags:(NSSet*)tags completion:(void (^)(NSError* error, NSString* registrationId))completion;
{
    if( deviceTokenData == nil)
    {
        if(completion)
        {
            completion([SBNotificationHubHelper errorForNullDeviceToken], nil);
        }
...
- (void) registerTemplateWithDeviceToken:(NSData*)deviceTokenData name:(NSString*)name jsonBodyTemplate:(NSString*)bodyTemplate expiryTemplate:(NSString*)expiryTemplate tags:(NSSet*)tags completion:(void (^)(NSError* error, NSString* registrationId))completion;
{
...
- (void) registerTemplateWithDeviceToken:(NSData*)deviceTokenData name:(NSString*)name jsonBodyTemplate:(NSString*)bodyTemplate expiryTemplate:(NSString*)expiryTemplate priorityTemplate:(NSString*)priorityTemplate tags:(NSSet*)tags completion:(void (^)(NSError* error, NSString* registrationId))completion;
{
    if( deviceTokenData == nil)
    {
        if(completion)
...
- (void)createOrUpdateWith:(NSString *)name payload:(NSString *)payload deviceToken:(NSString *)deviceToken completion:(void (^)(NSError *, NSString *))completion
{
    StoredRegistrationEntry* cached = [storageManager getStoredRegistrationEntryWithRegistrationName:name];
    if(cached == nil)
    {
        [self createRegistrationIdAndUpsert:name payload:payload deviceToken:deviceToken completion:^(NSError *error, NSString *registrationId)
         {
             ...
             else
             {
                 if(completion)
                 {
                     completion(error, registrationId);
                 }
             }
         }];
    }
    ...
    {
        [self upsertRegistrationWithName:name registrationId:cached.RegistrationId payload:payload completion:^(NSError *error, NSString *registrationId)
        {
            if( error != nil && [error code]== 410)
            {
...
- (void)createRegistrationIdAndUpsert:(NSString*)name payload:(NSString*)payload deviceToken:(NSString*)deviceToken completion:(void (^)(NSError*, NSString*))completion
{
...
- (void)upsertRegistrationWithName:(NSString*)name registrationId:(NSString*)registrationId payload:(NSString*)payload completion:(void (^)(NSError*, NSString*))completion
{
